...
const PAGE_GUIDE_ROWS = Array.from({ length: 80 }, (_, index) => 56 + index * 96);
const PAGE_GUIDE_COLUMNS = ['24px', '72px', '50%', 'calc(100% - 72px)', 'calc(100% - 24px)'];
const HERO_GUIDE_ROWS = Array.from({ length: 15 }, (_, index) => 52 + index * 72);

const PageGuides = () => (
  <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-[1]">
    {PAGE_GUIDE_ROWS.map((top) => (
      <span
        key={`h-${top}`}
        className="absolute left-0 right-0 border-t border-dotted border-primary/20"
        style={{ top }}
      />
    ))}
    {PAGE_GUIDE_COLUMNS.map((left) => (
      <span
        key={`v-${left}`}
        className="absolute top-0 bottom-0 border-l border-dotted border-primary/20"
        style={{ left }}
      />
    ))}
  </div>
...
const HeroGuides = () => (
  <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-[1]">
    {HERO_GUIDE_ROWS.map((top) => (
      <span
        key={`hero-h-${top}`}
        className="absolute left-0 right-0 border-t border-dotted border-bg/20"
        style={{ top }}
      />
    ))}
    {PAGE_GUIDE_COLUMNS.map((left) => (
      <span
        key={`hero-v-${left}`}
        className="absolute top-0 bottom-0 border-l border-dotted border-bg/20"
        style={{ left }}
      />
    ))}
  </div>
...
const Hero = ({ onTryForFree }) => {
  ...
  return (
    <section
      id="top"
      ref={heroRef}
      className="relative min-h-[100dvh] w-full overflow-hidden border-y border-dotted border-bg/22 pt-28 md:pt-32 pb-16 md:pb-20 px-6 md:px-14"
    >
      ...
      <div className="relative z-10 max-w-6xl mx-auto min-h-[74vh] grid grid-cols-1 lg:grid-cols-[1.02fr_0.98fr] gap-10 lg:gap-14 items-center">
        ...
        <div className="hero-reveal relative self-center">
          ...
          <div
            ref={previewRef}
            className="hero-tilt-card relative rounded-[2rem] border border-bg/24 bg-primary/72 p-6 md:p-7"
            onMouseMove={handlePreviewMove}
            onMouseLeave={handlePreviewLeave}
            style={{ '--tilt-x': '0deg', '--tilt-y': '0deg' }}
          >
            <div className="absolute inset-0 rounded-[2rem] border border-bg/16 pointer-events-none" />
            ...
            <div className="rounded-2xl border border-bg/14 bg-dark/65 p-4 md:p-5">
...
const HowItWorks = () => (
  <section id="how-it-works" className="relative w-full border-t border-dotted border-primary/25 bg-bg py-20 md:py-24 px-6 md:px-14">
...
const CraftPrinciples = () => (
  <section id="craft" className="relative w-full border-t border-dotted border-primary/25 bg-surface py-20 md:py-24 px-6 md:px-14">
    <div className="max-w-6xl mx-auto">
      <div className="max-w-2xl mb-12 md:mb-14">
...
const Examples = () => (
  <section id="examples" className="relative w-full border-t border-dotted border-primary/25 bg-bg py-20 md:py-24 px-6 md:px-14">
    <div className="max-w-6xl mx-auto">
      <div className="max-w-2xl mb-12 md:mb-14">
        <p className="font-mono text-xs uppercase tracking-[0.2em] text-accent mb-4">Examples</p>
        <h2 className="font-sans font-bold text-dark text-3xl md:text-5xl tracking-tight leading-tight">Animated signature outputs with a human edge.</h2>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {EXAMPLES.map((example) => (
          <article key={example.name} className="rounded-[1.6rem] border border-primary/12 bg-surface p-5 shadow-sm">
            <div className="rounded-[1.1rem] bg-bg/92 border border-primary/10 p-4 mb-4">
            ...
            <div className="flex items-center justify-between gap-3">
              <p className="font-sans font-semibold text-dark text-sm">{example.name}</p>
              <span className="rounded-md bg-accent text-bg px-2 py-1 font-mono text-[10px] tracking-[0.16em] uppercase">
                {example.type}
              </span>
            </div>
          </article>
...
const FinalCTA = ({ onTryForFree }) => (
  <section className="relative w-full border-y border-dotted border-primary/30 bg-accent py-20 md:py-28 px-6 md:px-14 text-center">
  ...
  <footer className="w-full bg-dark text-bg px-6 md:px-14 py-12 border-t border-bg/10">
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
      <div>
        ...
        <p className="font-serif italic text-[#FF8F64] text-lg">Rare signatures, moving with intent.</p>
...
function App({ onTryForFree }) {
  ...
  return (
    <>
      ...
      <main className="relative w-full overflow-x-hidden bg-bg">
        <PageGuides />
